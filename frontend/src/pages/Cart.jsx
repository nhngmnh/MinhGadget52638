...
const Cart = () => {
  ...
  const handleDelete = async () => {
    try {
      await axios.post(`${backendurl}/api/user/delete-cart/${deleteItemId}`, {
        headers: { token },
      });
      toast.success("Item removed from cart");
      setCart(cart.filter(item => item.id !== deleteItemId));
  ...
  {cart.map((item, index) => (
    ...
    >
      ...
      <div className="flex-1">
        <p className='text-2xl mb-2'>{item.itemData.name}&nbsp;&nbsp;&nbsp;{item.status==='processing'?<span className="text-gray-500 text-xs">&#128666; Delivering </span>   :(item.status==='shipped'?<span className="text-green-600 flex items-center gap-1 text-sm">
      <FaCheckCircle className="text-green-600 text-xs" /> Completed
    </span> :<span><FaTimesCircle className="text-red-500" /> Cancelled</span>)}</p>
        ...
        <p className="text-xs text-gray-500 mt-1">
          🕒 Ordered on: {new Date(new Date(item.deliveryDate).getTime() - 5 * 24 * 60 * 60 * 1000).toLocaleDateString()}
          </p>
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {!item.cancelled && !item.isCompleted && (
          <button onClick={()=>setShowConfirm(true)} className="px-4 py-2 text-sm text-white bg-red-500 hover:bg-red-600 transition rounded-lg">
            🗑 Delete
          </button>
        )}
